...
export class MenuFrontService {
  ...
  public find(filter: Partial<IMenuFront>, pagination?: IPaginationQuery): Observable<IMenuFront[] | IPaginationResponse<IMenuFront>> {
  ...
  public save(menuFront: IMenuFront): Observable<IMenuFront> {
    return this.http.post<IMenuFront>(`${environment.apiUrl}/menu-front`, menuFront);
  }

  public update(_id: string, menuFront: IMenuFront): Observable<IMenuFront> {
    return this.http.put<IMenuFront>(`${environment.apiUrl}/menu-front/${_id}`, menuFront);
  }

  public delete(menuFront: IMenuFront): Observable<boolean> {
  ...
  public getUserMenuFront(_id: string): Observable<IMenuFront[]> {
    return this.http.get<IMenuFront[]>(`${environment.apiUrl}/profile/get/user/menu-front/${_id}`);
  }

  public filterMenuItems(items: IMenuFront[], userRole: string): IMenuFront[] {
    if (!items?.length) {
      return null;
    }
