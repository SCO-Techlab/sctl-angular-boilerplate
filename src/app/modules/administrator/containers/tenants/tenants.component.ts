...
export class TenantsComponent {
  @ViewChild('filtersForm', { static: false }) filtersForm!: TenantsFiltersFormComponent;

  public showTable = false;
  public crudValues: ITenant[] = [];
  public crudState: CRUD_STATE = CRUD_STATE.VIEW;
  public crudConfig: ICrudComponent;
  public selectedItem: ITenant;
  public formValid: boolean = false;
  public filtersValue: Partial<ITenant> = {};
  ...
  ngOnInit() {
    this.translateService.stream('TENANTS')
  ...
  public onDeleteMultiple(values: string[]): void {
    ...
    this.confirmDialogService.confirm({
      ...
      accept: () => {
        ...
        this.tenantsService.deleteMultiple(values)
          .pipe(
  ...
  public async onExportData(): Promise<void> {
    const values = await new Promise<ITenant[]>((resolve) => {
      this.tenantsService.find(null)
        .pipe(takeUntilDestroyed(this.destroyRef$))
        .subscribe({
          next: (res: ITenant[]) => resolve(res ?? []),
          error: () => resolve([])
        })
    });
    ...
    const formatData = values.map((item: ITenant) => {
      return {
        ['_id']: item._id,
  ...
  public onCloseFormDialog(isSubmit: boolean): void {
    ...
    if (this.crudState === CRUD_STATE.NEW) {
      ...
      this.edit(this.selectedItemId, this.selectedItem);
    }
  }
  ...
  private getValues(): void {
    ...
    const filter: Partial<ITenant> = Object.values(cleanObject(this.filtersValue))?.length
      ? this.filtersValue
      : null;
    this.tenantsService.find(filter, this.paginationQuery)
      .pipe(
        takeUntilDestroyed(this.destroyRef$),
        finalize(() => this.showTable = true)
      )
      .subscribe((res: IPaginationResponse<ITenant>) => {
  ...
  private add(value: ITenant): void {
    this.spinnerService.show();
    this.tenantsService.save(value)
      .pipe(
        takeUntilDestroyed(this.destroyRef$),
        finalize(() => this.spinnerService.hide())
      )
  ...
  private delete(value: ITenant): void {
    if (!value) {
      return;
    }
    ...
    this.confirmDialogService.confirm({
      ...
      accept: () => {
        this.spinnerService.show();
        this.tenantsService.delete(value)
          .pipe(
            takeUntilDestroyed(this.destroyRef$),
            finalize(() => this.spinnerService.hide())
          )
  ...
  private edit(_id: string, value: ITenant): void {
    this.spinnerService.show();
    this.tenantsService.update(_id, value)
      .pipe(
        takeUntilDestroyed(this.destroyRef$),
        finalize(() => this.spinnerService.hide())
      )
  ...
  private setCrudConfig(): void {
    this.crudConfig = {
      ...
      cols: [
        ...
        {
          header: this.literals?.['COLS']['IS_ACTIVE'],
          field: 'isActive',
        ...
        {
          header: this.literals?.['COLS']['OWNER'],
          field: 'owner',
        ...
        },
        {
          ...
          options: { array: { dataKey: 'email', titleKeys: ['email'] } },
          headerStyles: 'max-width: 5rem',
          headerAlign: CRUD_COLUMN_ALIGNMENT.CENTER,
          fieldStyles: 'max-width: 5rem',
          fieldAlign: CRUD_COLUMN_ALIGNMENT.CENTER
      ...
      disabledButtons: {
        [CRUD_ACTIONS.NEW]: () => !this.userService.hasPermission(PERMISSIONS.TENANTS, PERMISSION_TYPE.CREATE),
        [CRUD_ACTIONS.DELETE_MULTIPLE]: () => !this.userService.hasPermission(PERMISSIONS.TENANTS, PERMISSION_TYPE.DELETE_BULK),
        [CRUD_ACTIONS.EXPORT]: () => !this.userService.hasPermission(PERMISSIONS.TENANTS, PERMISSION_TYPE.READ),
        [CRUD_ACTIONS.GLOBAL_FILTER]: () => !this.userService.hasPermission(PERMISSIONS.TENANTS, PERMISSION_TYPE.READ),
        [CRUD_ACTIONS.EDIT]: () => !this.userService.hasPermission(PERMISSIONS.TENANTS, PERMISSION_TYPE.UPDATE),
        [CRUD_ACTIONS.DELETE]: () => !this.userService.hasPermission(PERMISSIONS.TENANTS, PERMISSION_TYPE.DELETE),
        [CRUD_ACTIONS.CLEAR_FILTERS]: () => {
          return (
            !this.userService.hasPermission(PERMISSIONS.TENANTS, PERMISSION_TYPE.READ) ||
            Object.values(cleanObject(this.filtersValue))?.length === MAGIC_NUMBERS.N_0
          );
        },
        [CRUD_ACTIONS.SEARCH_FILTERS]: () => !this.userService.hasPermission(PERMISSIONS.TENANTS, PERMISSION_TYPE.READ),
      }
    };
  }
